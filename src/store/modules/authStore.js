  ...
  actions: {
    login: async function(context, payload) {
      return new Promise((resolve, reject) => {
        Parse.User.logIn(payload.username.toLowerCase(), payload.password).then(
          (res) => {
            ...
            context.commit('SET', ['SET_SESSION_TOKEN', temp.sessionToken])
    ...
    resetPassword: async function(payload) {
      return new Promise((resolve, reject) => {
        Parse.User.requestPasswordReset(payload.email).then((res) => {
          resolve(res)
        }, (err) => reject(err))
      })
    }
