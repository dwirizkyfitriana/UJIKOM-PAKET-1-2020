  ...
  actions: {
    ...
    fetchOperatorById: async function(context, payload) {
      return new Promise((resolve, reject) => {
        ...
        query.get(payload, { useMasterKey: true }).then((res) => {
    ...
    updateOperator: async function(context, payload) {
      return new Promise((resolve, reject) => {
        ...
        context.dispatch('fetchOperatorById', payload.objectId).then(res => {
          res.save({...payload, type: 'operator'}, {useMasterKey: true}).then(response => {
    ...
    deleteOperator: async function(context, payload) {
      return new Promise((resolve, reject) => {
        ...
        context.dispatch('fetchOperatorById', payload).then(res => {
          res.save({status: 500, deletedAt: new Date()}, {useMasterKey: true}).then(response => {
            response = JSON.parse(JSON.stringify(response))
            console.log(response)
