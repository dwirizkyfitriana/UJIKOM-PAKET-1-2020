    ...
    actions: {
      ...
      fetchStudentTransactions: async function(context, payload) {
        return new Promise(async (resolve,  reject) => {
          ...
          query.equalTo('student', {__type: 'Pointer', className: 'Student', objectId: payload})
          query.include('student.major')
          query.include('student.class')
          query.include('student.schoolYears')
      ...
      addTransaction: async function(context, payload) {
        return new Promise(async (resolve, reject) => {
          ...
          query.save({...payload, statusid: 1, serialCode: serial, 
            student: {__type: 'Pointer', className: 'Student', objectId: payload.student}
          }).then(res => {
            res = JSON.parse(JSON.stringify(res))

            // fetch student
            let student = context.rootGetters['students/getStudents']
            student = student.filter(item => item.objectId == res.student.objectId)
            student = student.reduce(acc => acc)
            res = {...res, student: student}

            console.log(res)

            context.commit('UNSHIFT', ['studentTransactions', res])
