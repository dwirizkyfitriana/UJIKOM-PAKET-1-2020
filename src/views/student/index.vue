    ...
    <v-app>
        ...
        <v-card class="ma-5 pa-5 pt-0" elevation="10" v-if="!loading">
            <v-row>
                <v-autocomplete
                    class="ma-5"
                    :items="filteredMajor ? filteredMajor : majors"
                    item-text="name"
                    item-value="objectId"
                    label="Pilih Jurusan"
                    v-model="major"
            ...
            <v-data-table
                :headers="headers"
                :items="filteredStudent ? filteredStudent : students"
                :search="search"
            >
...
export default {
    ...
    watch: {
        ...
        major(val) {
            if (val) {
                let res = this.students.filter(
                    (item) => item.major.objectId == val
                )
                this.filteredStudent = res

                // filtering classes by major
                let fileredClass = this.classes.filter(
                    (item) => item.major.objectId == val
                )
        ...
        klass(val) {
            ...
            if (val && this.major) {
                let res = this.students.filter(
                    (item) =>
                        item.class.objectId == val &&
                        item.major.objectId == this.major
                )
                this.filteredStudent = res
            }
